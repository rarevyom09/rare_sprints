...
const Video = () => {
  ...
  const handleTitleSave = async (url) => {
    ...
    try {
      const response = await fetch(`https://sprintsbyvyompadalia-rarevyom09.vercel.app/lesson-cards/${cardId}/update-video-title`, {
        method: 'POST',
        headers: {
  ...
  const saveEdits = async () => {
    ...
    try {
      const response = await fetch(`https://sprintsbyvyompadalia-rarevyom09.vercel.app/lesson-cards/${cardId}/update-details`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
  ...
  const saveNote = async () => {
    if (selectedUrl) {
      ...
      try {
        const response = await fetch(`https://sprintsbyvyompadalia-rarevyom09.vercel.app/lesson-cards/${cardId}/update-note`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'Content-Type': 'application/json',
  ...
  const addVideo = async () => {
    if (currentTitle && currentUrl) {
      ...
      try {
        const response = await fetch(`https://sprintsbyvyompadalia-rarevyom09.vercel.app/lesson-cards/${cardId}/add-link`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'Content-Type': 'application/json',
  ...
  const deleteVideo = (url) => {
    if (url) {
      ...
      if (confirmed) {
        fetch(`https://sprintsbyvyompadalia-rarevyom09.vercel.app/lesson-cards/${cardId}/delete-link`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'Content-Type': 'application/json',
    ...
    if (Array.isArray(videoItems)) {
      const updatedVideos = videoItems.map((item) => {
        if (item.link === url) {
          ...
          fetch(`https://sprintsbyvyompadalia-rarevyom09.vercel.app/lesson-cards/${cardId}/mark-link`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`,
              'Content-Type': 'application/json',
  ...
  useEffect(() => {
    async function fetchLinks() {
      setLoading(true);
      try {
        const response = await fetch(`https://sprintsbyvyompadalia-rarevyom09.vercel.app/lesson-cards/${cardId}`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  useEffect(() => {
    async function fetchCardInfo() {
      try {
        const response = await fetch(`https://sprintsbyvyompadalia-rarevyom09.vercel.app/lesson-cards/${cardId}/info`, {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
          },
